...
class BinaryImage {
private:
    class TT {//Вспомогательный класс
    public:
        T data;
        TT& operator=(bool value) {
            if (value)
                ...
                data = falseValue;
            return *this;
        }//присваиваем значение ячейке при работе с типом данных bool
        TT& operator=(T value) {
            if (value != trueValue && value != falseValue)
                throw "invalid value assignment";
            data = value;
            return *this;
        }//присваивание значения ячейке типа данных int(через конструктор)
        bool operator==(T value) {
            if (value == data)
                return true;
            else
                return false;
        }//смотрим какое значение в ячейке ()
...
public:
    ...
    bool operator()(int i, int j) const {//для констант
